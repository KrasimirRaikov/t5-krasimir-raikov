# The Tools Used in `bank`

## Introduction

`bank` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `bank` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`bank` uses git to manage its codebase. While in theory you don't have
to use Git once you download `bank`, this project makes the assumption
...
While `bank` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `bank` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `bank`; Grunt is
...
to get started with `bank`. But for those curious or looking to go a
...
Grunt is the engine behind `bank`. It's the magic that makes it move.
...
`bank` comes with a `bower.json` file that looks something like this:

```js
{
  "name": "t5-krasimir-raikov",
...
in `bank` by default in there to see what I mean.
