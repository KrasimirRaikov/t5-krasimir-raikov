# [bank](http://KrasimirRaikov.github.com/t5-krasimir-raikov) [![Build Status](https://api.travis-ci.org/ngbp/ngbp.png?branch=v0.3.2-release)](https://travis-ci.org/ngbp/ngbp)
...
```sh
$ git clone git://github.com/KrasimirRaikov/t5-krasimir-raikov
$ cd t5-krasimir-raikov
$ cd angular/frontend/
...
Finally, open `file:///path/to/t5-krasimir-raikov/build/index.html` in your browser.

Happy hacking!

## Purpose

`bank` is designed to make life easy by providing a basic framework
...
bank also comes prepackaged with the most popular design frameworks
...
The principal goal of `bank` is to set projects up for long-term
success.  So `bank` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `bank`.

But `bank` is not an example of an AngularJS app: this is a
...
getting `bank` up and running. Though `bank` is really simple
...
`bank` uses [Grunt](http://gruntjs.org) as its build system, so
...
```sh
$ git clone git://github.com/KrasimirRaikov/t5-krasimir-raikov my-project-name
$ cd my-project-name
```
...
There are many Bower packages used by `bank`, like Twitter Bootstrap
...
Technically, `bank` is now ready to go.
...
`bank`. What follows in this section is a quick introduction to the
...
* `delta:less` - When any `*.less` file within `src/` changes, the
  ...
  `build/assets/t5-krasimir-raikov.css`.
...
`bank` also includes [Live Reload](http://livereload.com/), so you no
...
default with `bank`) you need to tell Live Reload to allow it. Go to
...
See the [issues list](http://github.com/KrasimirRaikov/t5-krasimir-raikov/issues). And
